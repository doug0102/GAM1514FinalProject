...
void Level::update(double aDelta)
{
	...
	if(!m_PauseState)
	{
		...
		if(numberOfActiveEnemies <= 0)
		{
			...
			// Increase the round and increase the lives
			m_Round++;
			m_Lives++;
		}
		...
		// If there are no lives left, game over
		if(m_Lives <= 0)
		{
			// Only proceeds if the current screen is the game screen, NOT the level editor
			Screen* currentScreen = ScreenManager::getInstance()->getCurrentScreen();
...
void Level::spawnEnemies()
{
	...
	// If there are no valid spawn points GTFO 
	if(spawnableLocations.size() == 0)
	{
		return;
	}
	...
	for(int i = 0; i < m_Enemies.size(); i++)
	{
		if(m_Enemies.at(i) != NULL && spawnableLocations.at(i) != NULL)
		{
...
float Level::drawNumber(unsigned int number, float x, float y)
{
...
void Level::saveScore(int score)
{
	//Save a high score to JSON
	Json::Value root;

	// Open output stream
	std::ofstream outputStream;
	outputStream.open("highscore", std::ofstream::out);
...
void Level::loadScore()
{
	std::ifstream inputStream;
	inputStream.open("highscore", std::ifstream::in);
