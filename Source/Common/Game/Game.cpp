...
Game::Game()
{
	...
	m_TowersMenu->addButton(new UIToggle("MenuTileTurret"));
	m_TowersMenu->addButton(new UIToggle("MenuTileSlow"));
	...
	m_ControlMenu->addButton(new UIButton("FastForward"));
...
void Game::keyUpEvent(int keyCode)
{
	...
	else if(keyCode == KEYCODE_SPACE)
	{
		if(m_Level != NULL)
		{
			m_Level->m_PauseState == true ? m_Level->m_PauseState = false : m_Level->m_PauseState = true;
		}
	}
	else
	{
...
void Game::sideMenuButtonAction(UISideMenu* sideMenu, UIButton* button, int buttonIndex)
{
	if(sideMenu == m_ControlMenu)
	{
		if(m_Level != NULL)
		{
			...
			case 3:
				// Fastforward game speed
				
				if(m_Level != NULL)
				{
					if(m_Level->m_LevelSpeed == 1.0)
					{
						m_Level->m_LevelSpeed = 2.0;
					}
					else if(m_Level->m_LevelSpeed == 2.0)
					{
						m_Level->m_LevelSpeed = 5.0;
					}
					else
					{
						m_Level->m_LevelSpeed = 1.0;
					}
				}
				break;
			}
